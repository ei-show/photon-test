...
class DemoLoadBalancing extends Photon.LoadBalancing.LoadBalancingClient {
    ...
    constructor() {
        ...
        this.logger.info("Photon Version: " + Photon.Version);
    ...
    setupUI() {
        ...
        btnJoinOrCreate.onclick = (ev) => {
            if (this.isInLobby()) {
                ...
                this.joinRoom(gameId.value.length > 0 ? gameId.value : undefined, { createIfNotExists: true, expectedUsers: expectedUsers.value.length > 0 ? expectedUsers.value.split(",") : undefined },
                    { emptyRoomLiveTime: 20000, suspendedPlayerLiveTime: 20000, maxPlayers: 6 }
                );
        ...
        btn.onclick = (ev) => {
            ...
            this.myRoom().setEmptyRoomLiveTime(this.myRoom().emptyRoomLiveTime + 1000);
            this.myRoom().setSuspendedPlayerLiveTime(this.myRoom().suspendedPlayerLiveTime + 2000);
